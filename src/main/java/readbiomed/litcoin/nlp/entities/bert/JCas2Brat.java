...
public class JCas2Brat extends JCasAnnotator_ImplBase {
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
		...
		for (Relation relation : JCasUtil.select(jCas, Relation.class)) {
			...
			d.addRelation(relationId,
					new au.com.nicta.csp.brateval.Relation(relationId, relation.getRelationType(), "arg1",
							d.getEntity(relation.getArg1().getMentionId()), "arg2",
							d.getEntity(relation.getArg2().getMentionId()), "ann"));
		...
		for (Event event : JCasUtil.select(jCas, Event.class)) {
			...
			for (int i = 0; i < event.getRoles().size(); i++) {
				EventRole eventRole = event.getRoles(i);
				arguments.add(eventRole.getRole() + ":" + eventRole.getEntity().getMentionId());
			}

			d.addEvent(eventId, new au.com.nicta.csp.brateval.Event(eventId, event.getEventType(),
					event.getTrigger().getMentionId(), arguments, "ann"));
